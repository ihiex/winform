    ...
    public partial class Assemble_Form : Form
    {
        ...
        private bool CheckInputBarcode(TextBox txtBox)
        {
            try
            {
                ...
                //if (txtBox.Name.Replace("txt", "") == "1" && ScanType == "1")
                //{
                //    Result = PartSelectSVC.MESAssembleCheckMianSN(Com_PO.EditValue.ToString(), List_Login.LineID,
                //        List_Login.StationID, List_Login.StationTypeID, barcode);
                //}
                ...
                //        //Machine校验
                //        string StatusID = "";
                //        string StationTypeFromID = "";
                //        string StationTypeToID = "";
                //        if (txtBox.Name.Replace("txt", "") == "1" || ScanType == "4")
                //        {
                //            StatusID = "1";
                //            StationTypeFromID = List_Login.StationTypeID.ToString();
                //        }
                //        else
                //        {
                //            StatusID = "2";
                //            StationTypeToID = List_Login.StationTypeID.ToString();
                //        }
                //        string xmlExtraData = string.Format("<ExtraData StatusID=\"{0}\" StationTypeFromID =\"{1}\" StationTypeToID =\"{2}\"></ExtraData>",
                //                                                StatusID, StationTypeFromID, StationTypeToID);
                //        string xmlPartStr = "<Part PartID=\"" + PartID + "\"> </Part>";
                        
                //        PartSelectSVC.uspCallProcedure("uspMachineToolingCheck", barcode,
                //                                                                null, xmlPartStr, null, xmlExtraData, null, ref Result);
        ...
        public bool SubmitData()
        {
            ...
            foreach (Control ctr in panelSpool.Controls)
            {
                if (ctr.Name.Substring(0, 3) == "txt")
                {
                    ...
                    //#region 如果存在虚拟类型条码校验数量是否超过最大限制 ---检查 已经校验暂时不用校验
                    //if (tagList[1].ToString() == "3" || tagList[1].ToString() == "4")
                    //{
                    //    string outString = string.Empty;
                    //    PartSelectSVC.uspCallProcedure("uspMachineToolingCheck", txt.Text.ToUpper(), null, null, null, null, null, ref outString);
                    //    if (outString != "OK")
                    //    {
                    ...
                    if (ctr.Name.Replace("txt", "") == "1")
                    {
                        ...
                        else
                        {
                            ...
                            v_mesUnit.ID = Convert.ToInt32(MainUnitID);
