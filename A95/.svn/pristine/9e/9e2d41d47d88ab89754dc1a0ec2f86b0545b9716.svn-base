...
namespace App.MyMES
{
    public partial class Tooling_Form : DevExpress.XtraEditors.XtraForm
    {
        ...
        ImesPartSVCClient PartSVC = ImesPartFactory.CreateServerClient();
        ...
        private Boolean GetIsRoute(int Scan_StationID, DataTable DT_Unit)
        {
            Boolean B_Result = true;
            string S_PartID = DT_Unit.Rows[0]["PartID"].ToString();
            string S_PartFamilyID = DT_Unit.Rows[0]["PartFamilyID"].ToString();
            string ProductionOrderID= DT_Unit.Rows[0]["ProductionOrderID"].ToString();
            //最后扫描工序
            string S_StationID = DT_Unit.Rows[0]["StationID"].ToString();
            //获取此料工序路径
            int RouteID = PartSelectSVC.GetRouteID(List_Login.LineID.ToString(), S_PartID, S_PartFamilyID, ProductionOrderID);
        ...
        private void Edt_ToolSN_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Enter)
            {
                ...
                mesPart mesPart = PartSVC.Get(Convert.ToInt32(S_PartID));
                string S_POID = DT_ProductionOrder.Rows[0]["ID"].ToString();
                string S_PO_StatusID = DT_ProductionOrder.Rows[0]["StatusID"].ToString();
                string ProductionOrderID = DT_ProductionOrder.Rows[0]["ProductionOrderID"].ToString();
                ...
                if (S_SN != "")
                {
                    ...
                    //获取此料工序路径
                    int RouteID = PartSelectSVC.GetRouteID(List_Login.LineID.ToString(), S_PartID, mesPart.PartFamilyID.ToString(), ProductionOrderID);
                    DataTable DT_Route = PartSelectSVC.GetRoute("", RouteID).Tables[0];
                    //var v_Route_Sacn = from c in DT_Route.AsEnumerable()
