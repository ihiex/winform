    ...
    public class PartSelectDAL
    {
        ...
        public DataSet GetRoute(string PartID, string S_RouteSequence, string LineID, int StationTypeID)
        {
            string S_Sql_Line = @"select B.ID,B.Name,B.Description RouteName,
                            ...
                            from 
                                (select * from  mesRouteDetail)A
	                            join (select * from mesRoute) B on A.RouteID=B.ID  		  	
	                            join (select * from mesRouteMap)C on B.ID=C.RouteID
        ...
        public string MESAssembleCheckVirtualSN(string SN, string PartID)
        {
            ...
            if (S_StatusID != "2")
            {
                return string.Format("SN：{0} 状态不符，请绑定批次！", SN);
            }
            string S_MachineFamilyID = DT_Machine.Rows[0]["MachineFamilyID"].ToString();
            string S_MachinePartID = DT_Machine.Rows[0]["PartID"].ToString();
            ...
            {
                return string.Format("SN：{0} 设备未分配任何料号！", SN);
            }
            ...
            if(PartID != S_MapPartID)
            {
                ...
                return string.Format("SN：{0} 子料和主料未绑定BOM关系！", SN);
            }
            ...
            {
                return string.Format("SN：{0} 未找到子料绑定的批次号！", SN);
            }
