...
namespace MESLabel
{
	internal class Day : MESFuncObject
	{
		public Day(string name) : base(name, 0)
		{
		}
		...
		public override string Evaluate(ArrayList Params)
		{
			...
			if (Params.Count == this.ParamNum)
			{
				try
				{
					int num = DateAndTime.DatePart(DateInterval.Day, DateTime.Now, FirstDayOfWeek.Sunday, FirstWeekOfYear.Jan1);
					if (num < 10)
					{
