...
namespace App.MyMES
{
    public partial class PrintSNOffline_Form : FrmBase2
    {
        ...
        private void Btn_CreateSN_Click(object sender, EventArgs e)
        {
            ...
            //DataSet DS_LabelCMD = public_.GetLabelCMD(PartSelectSVC, S_StationTypeID, S_PartFamilyID, S_PartID, S_ProductionOrderID, S_LoginLineID);
            //DataTable DT_LabelCMD = null;
            //if (DS_LabelCMD != null && DS_LabelCMD.Tables.Count > 0)
            //{
            //    if (DS_LabelCMD.Tables[0].Rows.Count > 0)
            //    {
            //        DT_LabelCMD = DS_LabelCMD.Tables[0];
            //    }
            //}
            ...
            //string ss = Com_LineGroup.EditValue.ToString();
            ...
            if (result == "1" && dsSN != null && dsSN.Tables.Count > 0)
            {
                ...
                //string S_PrintResult = Public_.PrintCodeSoftSN(PartSelectSVC, S_LabelName, DT_PrintSn, I_PartID, DT_LabelCMD);
        ...
        private void Btn_Reprint_Click(object sender, EventArgs e)
        {
            ...
            if (S_PrintResult != "OK")
            {
                ...
                PartSelectSVC.AddmesReprintHistory("NG", Edt_SN.Text.Trim(), List_Login.StationID.ToString());
            }
            else
            {
                MessageInfo.Add_Info_MSG(Edt_MSG, "10032", "OK", List_Login.Language, new string[] { Edt_SN.Text.Trim() });
                Edt_Num.Text = "";
                PartSelectSVC.AddmesReprintHistory("OK", Edt_SN.Text.Trim(), List_Login.StationID.ToString());
            }
        ...
        {
            ZebraPrint.ZebraPrintHelper.SendFileToPrinter("\\\\192.168.50.164\\p001", "D:\\Labels\\S01\\FG.txt");
        }
        ...
        private void PrintQTY()
        {
            ...
            DataRow[] dr1 = dtStation.Select("Name='PrintQTY'");
            if (dr1.Length > 0)
            {
                string S_Calue = dr1[0]["Value"].ToString();
                if (dr1 != null && dr1.Length != 0 && S_Calue != "")
                {
                    I_PrintQTY = Convert.ToInt32(S_Calue);
                }
            }
            else
            {
