...
namespace App.DBServerDAL
{
	public class   mesMachineDAL
	{
        ...
        public void MesModMachineBySNStationTypeID(string MachineSN,int StationTypeID)
        {
            ...
            S_Sql = string.Format("select * from mesMachine where SN='{0}'", MachineSN);
            DataTable dts = SqlServerHelper.ExecuteDataTable(S_Sql);
            int WarningStatus = Convert.ToInt32(dts.Rows[0]["WarningStatus"].ToString());
            int StationFrom = Convert.ToInt32(dts.Rows[0]["ValidFrom"].ToString());
            int StationTo = Convert.ToInt32(dts.Rows[0]["ValidTo"].ToString());

            if (dts != null || dts.Rows.Count > 0)
            {
                if (StationTypeID == StationFrom)
                {
                ...
                if (StationTypeID == StationTo)
                {
                    switch (WarningStatus)
                    {
                        case 0:
                            ...
                            break;
                        case 1:
                            S_Sql = string.Format("Update mesMachine set LastRuningTime=GETDATE(),RuningQuantity=ISNULL(RuningQuantity,0)+1 where SN = '{0}'", MachineSN);
                            break;
                        case 2:
                            S_Sql = string.Format("Update mesMachine set LastRuningTime=GETDATE(),StatusID=1,RuningQuantity=ISNULL(RuningQuantity,0)+1 where SN = '{0}'", MachineSN);
                            break;
                        default:
                            break;
                    }
