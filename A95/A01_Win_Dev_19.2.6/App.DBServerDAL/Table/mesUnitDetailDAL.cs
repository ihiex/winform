...
namespace App.DBServerDAL
{
    public class mesUnitDetailDAL
    {
        ...
        {
            DataTable dt = SqlServerHelper.ExecuteDataTable("SELECT * FROM mesUnitDetail WHERE unitid=@unitid", new SqlParameter("@unitid", unitid));
            if (dt.Rows.Count > 1)
            {
                throw new Exception("more than 1 row was found");
            }
        ...
        private static mesUnitDetail ToModel(DataRow row)
        {
            ...
            model.KitSerialNumber = row["KitSerialNumber"].ToString();
            //model.InmostPackageID = (int)row["InmostPackageID"];
            //model.OutmostPackageID = (int)row["OutmostPackageID"];
            model.reserved_01 = row["reserved_01"].ToString();
            model.reserved_02 = row["reserved_02"].ToString();
            model.reserved_03 = row["reserved_03"].ToString();
            model.reserved_04 = row["reserved_04"].ToString();
            model.reserved_05 = row["reserved_05"].ToString();
        ...
        public DataSet MesGetBatchIDByBarcodeSN(string BarcodeSN)
        {
            ...
            DataSet ds = null;
            strSql = string.Format("select * from mesMachine where SN='{0}' AND StatusID=2", BarcodeSN);
            DataTable dts = SqlServerHelper.ExecuteDataTable(strSql);
            if (dts != null && dts.Rows.Count > 0)
            {
                strSql = string.Format("SELECT top 1 reserved_02,UnitID FROM mesUnitDetail WHERE reserved_01='{0}' and reserved_03=1 order by id desc", BarcodeSN);
        ...
        public string MesGetFGSNByUPCSN(string UPCSN)
        {
            ...
            string strSql = string.Empty;
            strSql = string.Format(@"SELECT B.Value SerialNumber FROM  mesUnitDetail A 
                INNER JOIN mesSerialNumber B ON A.UnitID = B.UnitID WHERE A.KitSerialNumber = '{0}'", UPCSN);
            DataTable dts = SqlServerHelper.ExecuteDataTable(strSql);
            if (dts != null && dts.Rows.Count > 0)
            {
                FGSN = dts.Rows[0]["SerialNumber"].ToString();
            }
            return FGSN;
        }
        ...
        {
            string sql = string.Format("UPDATE mesUnitDetail SET KitSerialNumber='{0}' WHERE UnitID={1}", KitSerialnumber, UnitID);
