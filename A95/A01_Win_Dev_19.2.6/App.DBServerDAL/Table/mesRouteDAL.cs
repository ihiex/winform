...
namespace App.DBServerDAL
{
	public class   mesRouteDAL
	{
        ...
        public int MESGetRounterID(int LineID,int PartID, int PartFamilyID,int ProductionOrderID)
        {
            int routeID = 0;
            string S_Sql = string.Format("SELECT DBO.ufnRTEGetRouteID({0},{1},{2},{3})", LineID, PartID, PartFamilyID, ProductionOrderID);
        ...
        {
            string Str_Sql= string.Format(@"SELECT A.LineID,A.PartID,B.PartFamilyID,A.ProductionOrderID FROM mesUnit A 
            ...
            int ProductionOrderID = Convert.ToInt32(dtUnt.Rows[0]["ProductionOrderID"]);
            int RouteID = MESGetRounterID(LineID, partID, PartFamilyID, ProductionOrderID);
