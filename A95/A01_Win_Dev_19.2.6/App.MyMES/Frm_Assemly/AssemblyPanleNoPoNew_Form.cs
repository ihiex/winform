...
namespace App.MyMES
{
    public partial class AssemblyPanleNoPoNew_Form : FrmBase
    {
        //2021-07-02  测试
        ...
        public override void FrmBase_Load(object sender, EventArgs e)
        {
            ...
            {
                ScanTypeNew = drScanType[0]["Content"].ToString();
            }
        ...
        public override void Btn_ConfirmPO_Click(object sender, EventArgs e)
        {
            ...
            //    //获取参数1值
            //    DataSet dtSet = PartSelectSVC.GetProductionOrderDetailDef(ProductionOrderID);
            ...
            //DataSet ds = PartSelectSVC.MESGetBomPartInfo(Convert.ToInt32(PartID),
        ...
        private bool VerifyMianCode()
        {
            ...
            DataSet dsCOF = PartSelectSVC.GetluPODetailDef(Convert.ToInt32(ProductionOrderID), "COF");
            if (dsCOF != null && dsCOF.Tables.Count > 0 && dsCOF.Tables[0].Rows.Count > 0)
            {
                COF = dsCOF.Tables[0].Rows[0]["Content"].ToString() == "1";
            }
            ...
            //工单数量检查
            PartSelectSVC.uspCallProcedure("uspPONumberCheck", ProductionOrderID,
                    null, null, null, null, "1", ref outString);
            if (outString != "1")
            {
                string ProMsg = MessageInfo.GetMsgByCode(outString, List_Login.Language);
                MessageInfo.Add_Info_MSG(Edt_MSG, "20011", "NG", List_Login.Language, new string[] { ProductionOrderID, ProMsg }, outString);
                txtMainSN.Text = string.Empty;
                return false;
            }

            if (PartID != PartOld || string.IsNullOrEmpty(PartOld))
            {
                ...
                if (ds == null || ds.Tables.Count == 0 || ds.Tables[0].Rows.Count < 2)
                {
                    MessageInfo.Add_Info_MSG(Edt_MSG, "20023", "NG", List_Login.Language, new string[] { PartID });
                    return false;
                }
        ...
        private void InitialContrl()
        {
            ...
            txtMainSN.Focus();
            GrpControlInputData.CustomHeaderButtons[0].Properties.Caption = "1/1";
        }
        ...
        public bool SubmitData()
        {
            ...
            string msg = string.Empty;
            string S_POID = ProductionOrderID;
            string S_PartID = PartID;
            string S_PartFamilyID = PartFamilyID;
            string S_UnitStateID = public_.GetluUnitStatusID(S_PartID, S_PartFamilyID, List_Login.StationTypeID,
                List_Login.LineID.ToString(), ProductionOrderID, Com_luUnitStatus.EditValue.ToString());
            ...
            for (int i = 0; i < dtScanData.Rows.Count; i++)
            {
                ...
                if (i == 0)
                {
                   ...
                   // string S_POPartID = DT_ProductionOrder.Rows[0]["PartID"].ToString();
                    ...
                    sqlValue = sqlValue + " insert into mesHistory(UnitID, UnitStateID, EmployeeID, StationID, EnterTime, ExitTime, ProductionOrderID, PartID, LooperCount, StatusID) " +
                                          ...
                                          "', '" + List_Login.StationID + "', getdate(), GETDATE(), '" + Convert.ToInt32(S_POID) + "', '" + Convert.ToInt32(S_PartID) + "', '1', 1)";
                ...
                if (ScanType == "2" || ScanType == "7" || ScanType == "3")
                {
                    ...
                    string ResultStr = PartSelectSVC.ModMesMaterialConsumeInfo(List_Login, BatchType, SN, MachineSN,
                                    Convert.ToInt32(PartID), Convert.ToInt32(ProductionOrderID));
                    if (ResultStr != "1")
                    {
            ...
            try
            {
                ...
                if (string.IsNullOrEmpty(S_FG_SN))
                {
                    ...
                    string S_FormatSN = PartSelectSVC.mesGetSNFormatIDByList(S_PartID, S_PartFamilyID,
                        List_Login.LineID.ToString(), ProductionOrderID, List_Login.StationTypeID.ToString());
                    ...
                    string xmlProdOrder = "'<ProdOrder ProductionOrder=" + "\"" + ProductionOrderID + "\"" + "> </ProdOrder>'";
