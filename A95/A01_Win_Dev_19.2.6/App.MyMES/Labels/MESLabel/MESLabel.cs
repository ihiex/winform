...
namespace MESLabel
{
    public class MESLabel
    {
        ...
        int I_Port = 6899;
        Socket clientSocket;
        static BarTender.Application BTApp;
        static BarTender.Format BTFormat;
        ...
        public MESLabel(string name, MESLabel.LabelType type, MESLabel.PrinterType @out)
        {
            ...
            try
            {
                ...
                if (this.m_out == MESLabel.PrinterType.BartenderCom)
                {
                    try
                    {
                        if (BTApp == null)
        ...
        public virtual void Print(DataTable DT_Print, PartSelectSVCClient PartSelectSVC)
        {
            switch (this.m_out)
            {
                case MESLabel.PrinterType.ZPLDirect:
                    //this.ZPLDirectPrint();
                    ZPLPrint(DT_Print, PartSelectSVC);
                    break;
                case MESLabel.PrinterType.TmpZPL:
                    //this.ZPLTemplate();
                    ZPLPrintOneSN(DT_Print, PartSelectSVC);
                    break;
                ...
                case MESLabel.PrinterType.Bartender:
                    ...
                    break;
                case MESLabel.PrinterType.BartenderCom:
                    BartenderComPrint(DT_Print, PartSelectSVC);
                    break;
            }
        }

        //public virtual void Print(DataTable DT_Print, PartSelectSVCClient PartSelectSVC, DataTable DR_LabelCMD)
        //{
        //	switch (this.m_out)
        //	{
        //	    case MESLabel.PrinterType.ZPLDirect:
        ...
        protected virtual void ZPLPrint(DataTable DT_Print, PartSelectSVCClient PartSelectSVC)
        {
            DataTable DR_LabelCMD = PartSelectSVC.LabelNameToLabelCMD(S_LabelName).Tables[0];

            DataRow[] List_DR = DR_LabelCMD.Select("Type=1");
            int I_ColCount = List_DR.Length;

            //DataRow[] List_PageTop = DR_LabelCMD.Select("Type=100");
            //string S_PageTop = List_PageTop[0]["Cmd"].ToString();

            DataRow[] List_Top = DR_LabelCMD.Select("Type=200");
            string S_Top = List_Top[0]["Cmd"].ToString();

            DataRow[] List_Bot = DR_LabelCMD.Select("Type=300");
            string S_Bot = List_Bot[0]["Cmd"].ToString();

            string S_Value = "";

            try
            {
                for (int i = 0; i < DT_Print.Rows.Count; i += I_ColCount)
                {
                    string S_GroupValue = "";
                    ...
                    try
                    {
                        ...
                        {
                            DataTable DTxml = PartSelectSVC.GetLBLGenLabel(DT_Print.Rows[i + j]["SN"].ToString(), S_LabelName).Tables[0];
                            string S_xml = DTtoXML(DTxml);

                            MESLabel.m_meslabel = null;
                            MESLabel.m_meslabel = MESLabel.ParseXML(S_xml, false, "");

                            string S_Cmd = List_DR[j]["Cmd"].ToString();
                                    ...
                                    foreach (var item in m_formats)
                                    {
                                        ...
                                        try
                                        {
                                            foreach (var item2 in labelFormat.Fields)
                                            {
                                                ...
                                                S_Cmd = S_Cmd.Replace("%" + labelField.Name + "%", labelField.Value.ToString());
                                            }
                            ...
                            DT_Print.Rows[i + j]["PrintTime"] = DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss");
                        }
                    }
                    catch
                    {
                    ...
                    File.Delete(this.TFP);

                    //S_Value = S_PageTop + "\r\n" + S_Value;
                    StreamWriter sw = new StreamWriter(this.TFP, true, Encoding.UTF8);
                    sw.Write(S_Value);
                    sw.Close();
                    ZebraPrintHelper.SendFileToPrinter(MESLabel.PrinterName, this.TFP);
                    Thread.Sleep(200);
                }
            }
            catch
        ...
        // 一排只能打一个SN
        protected virtual void ZPLPrintOneSN(DataTable DT_Print, PartSelectSVCClient PartSelectSVC)
        {
            for (int i = 0; i < DT_Print.Rows.Count; i++)
            {
                DataTable DTxml = PartSelectSVC.GetLBLGenLabel(DT_Print.Rows[i]["SN"].ToString(), S_LabelName).Tables[0];
        ...
        protected virtual void CodeSoftPrint(DataTable DT_Print, PartSelectSVCClient PartSelectSVC)
        {
            ...
            try
            {
                ...
                LabSN.Documents.Open(S_Path_Lab, true);
                    ...
                    try
                    {
                        foreach (var item in m_formats)
                        {
                            ...
                            try
                            {
                                ...
                                DT_Print.Rows[i]["PrintTime"] = DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss");
                                doc.PrintLabel(1);
                                //doc.FormFeed();  2023-06-07  FG 不能一行多个了
                            }
                            catch (Exception ex)
                            {
                                doc.Close();
        ...
        protected virtual void BartenderPrint(DataTable DT_Print, PartSelectSVCClient PartSelectSVC)
        {
            ...
            string TargetPathFile = DT_Print.Rows[0]["TargetPath"].ToString();
            string TargetPath = string.Empty;
            string TargetFile = string.Empty;
            if (string.IsNullOrEmpty(TargetPathFile))
            {
                TargetPath = "D:\\Labels";
                TargetFile = "BarTender.txt";
                TargetPathFile = TargetPath + "\\" + TargetFile;
            }
            else
            {
                TargetPath = Path.GetDirectoryName(TargetPathFile);
                TargetFile = Path.GetFileName(TargetPathFile);
            }
            ...
            {
                MessageBox.Show("The file path was not found(未找到文件路径)", "ERROR", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }

            string S_DBPath = TargetPathFile;
            ...
            //    INI = new App.MyMES.MyINI("D:\\Labels\\BarTender.ini");
            //}
                ...
                if (m_formats.Count == 1)
                {
                    try
                    {
                        foreach (var item in m_formats)
                        {
                            ...
                            try
                            {
                                //string S_label_Log = DateTime.Now.ToString("yyyy-MM-dd HH_mm_ss") + ".txt";
                                if (I_Start == 0)
                                {
                                    string S_Title = "";
                                    foreach (var item2 in labelFormat.Fields)
                                    ...
                                    //File.AppendAllText("D:\\Labels\\Label_Log\\" + S_label_Log, S_Title + "\r\n");
                                }
                                ...
                                //File.AppendAllText("D:\\Labels\\Label_Log\\"+ S_label_Log, S_Value + "\r\n");
            ...
            try
            {
                ...
                //{
                //    socketTCPClient = new SocketTCPClient("127.0.0.1", I_Port);
                //}
                //else
                //{
                ...
                //if (!socketTCPClient.Connected)
                //{
                //    socketTCPClient.InitSocket("127.0.0.1", I_Port);
                //    socketTCPClient.ConnectServer();
                //}

                //Thread.Sleep(200);
                //socketTCPClient.SendCommand(Encoding.Default.GetBytes("Print"+ S_TmpPath));
                //Thread.Sleep(200);
                //string command = ReceiveMessage(socketTCPClient);
                ...
                string mesaage = "Print" + S_TmpPath;
                int length = mesaage.Length;
                Thread.Sleep(200);
                SocketTCPClient.Send(length.ToString().PadLeft(4, '0') + mesaage);
                byte[] ReceiveMessage = new byte[1024];
                Thread.Sleep(200);

                int resv = 0;
                string command = "";
                Boolean B_Socket = false;

                int I_SocketTimeout = 20;
                try
                {
                    Configuration config = ConfigurationManager.OpenExeConfiguration(ConfigurationUserLevel.None);
                    string S_SocketTimeout = config.AppSettings.Settings["SocketTimeout"].Value.Trim();

                    if (S_SocketTimeout != "")
                    {
                        I_SocketTimeout = Convert.ToInt32(S_SocketTimeout);
                    }
                }
                catch
                {
                ...
                while (B_Socket == false)
                {
                    B_Socket = true;
                    try
                    {
                        resv = SocketTCPClient._clientSocket.Receive(ReceiveMessage);
                        command = Encoding.GetEncoding("GBK").GetString(ReceiveMessage, 0, resv);
                    ...
                    double I_TotalSeconds = Math.Round(TS.TotalSeconds, 0);
                    if (I_TotalSeconds >= I_SocketTimeout)
                    {
                        B_Socket = true;
                    }
                }
                ...
                if (command != "Delete")
                {
                    MessageBox.Show("Bartender打印失败,错误信息:" + command, "ERROR", MessageBoxButtons.OK, MessageBoxIcon.Error);
                }
                //Thread.Sleep(200);
            ...
            {
                //if (socketTCPClient != null && socketTCPClient.Connected)
                //{
                //    socketTCPClient.Close();
                //}
            }
        ...
        protected virtual void BartenderComPrint(DataTable DT_Print, PartSelectSVCClient PartSelectSVC)
        {
            string S_TmpPath = DT_Print.Rows[0]["TmpPath"].ToString();
            string TargetPathFile = DT_Print.Rows[0]["TargetPath"].ToString();
            string TargetPath = string.Empty;
            ...
            try
            {
                ...
                if (BTFormat == null)
                {
                    BTFormat = BTApp.Formats.Open(S_Path_Lab, false, "");
                }
                            ...
                            try
                            {
                                //if (I_Start == 0)
                                {
                                    foreach (var item2 in labelFormat.Fields)
                                    {
                                        MESLabel.LabelField labelField = (MESLabel.LabelField)item2;
                                        labelField.table = table;
                                        BTFormat.SetNamedSubStringValue(labelField.Name.ToString(), labelField.Value.ToString());
                                    }
        ...
        ///// </summary>
        ///// <returns></returns>
        //public string ReceiveMessage(SocketTCPClient socketTCPClient)
        //{
        //    string data = string.Empty;
        //    byte[] buffer = new byte[1024];
        //    if (socketTCPClient != null && socketTCPClient.Connected)
        //    {
        //        int length = socketTCPClient.socket.Receive(buffer);
        //        if (length > 0)
        //        {
        //            data = Encoding.Default.GetString(buffer, 0, length).Replace("\r", "").Replace("\n", "");
        //        }
        //    }
        //    return data;
        //}
            ...
            try
            {
                if (!this.IsIPAddress(this.TFP))
                {
                    ...
                    if (sb.ToString() != "" & sb != null)
                    {
                        ...
                        if (streamWriter != null) { streamWriter.Close(); streamWriter.Dispose(); }
        ...
        //    MESLabel.m_meslabel = MESLabel.ParseXML(xml, useCustomLabelDLL, customLabelDLL);
        //    MESLabel.m_meslabel.Print(DT_Print, PartSelectSVC, DR_LabelCMD);
        //    return true;
        //}
